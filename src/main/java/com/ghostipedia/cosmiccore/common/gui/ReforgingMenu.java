package com.ghostipedia.cosmiccore.common.gui;
...
public class ReforgingMenu extends BlockEntityMenu<ReforgingTableTile> {
    ...
    public final RandomSource random = new XoroshiroRandomSource(0);
    protected final int[] seed = new int[2];
    protected final int[] costs = new int[3];
    protected DataSlot needsReset = DataSlot.standalone();

    public ReforgingMenu(MenuType<? extends ReforgingMenu> type, int id, Inventory inv, FriendlyByteBuf pos) {
        super(type, id, inv, pos.readBlockPos());
