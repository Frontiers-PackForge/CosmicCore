...
public class WirelessComputationTransmitter extends WorkableElectricMultiblockMachine
    ...
    public WirelessComputationTransmitter(IMachineBlockEntity holder, Object... args) {
        super(holder, args);
    ...
    private int calculateEnergyUsage() {
        int receivers = getOpticalReceivers().size();
    ...
    private void addHatchesToWirelessNetwork() {
        WirelessCwuStore.addHatches(getTeamUUID(), getOpticalReceivers());
    }

    private void removeHatchesFromWirelessNetwork() {
        WirelessCwuStore.removeHatches(getTeamUUID(), getOpticalReceivers());
    }

    private List<IOpticalComputationReceiver> getOpticalReceivers() {
        List<IOpticalComputationReceiver> receivers = new ArrayList<>();

        for (var part : this.getParts()) {
            Block block = part.self().getBlockState().getBlock();
            if (part instanceof IOpticalComputationReceiver receiver &&
                    PartAbility.COMPUTATION_DATA_RECEPTION.isApplicable(block)) {
