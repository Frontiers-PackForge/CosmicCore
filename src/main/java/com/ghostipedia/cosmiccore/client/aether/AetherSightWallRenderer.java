...
public final class AetherSightWallRenderer {
    ...
    private static final RenderType STORM_WALL_TYPE = CosmicCoreRenderTypes.aetherStormCurrent();
    ...
    private AetherSightWallRenderer() {}

    @SubscribeEvent
    public static void onRenderLevel(RenderLevelStageEvent event) {
        if (event.getStage() != RenderLevelStageEvent.Stage.AFTER_TRANSLUCENT_BLOCKS) return;
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.level == null || !minecraft.level.dimension().location().equals(AETHER)) return;
