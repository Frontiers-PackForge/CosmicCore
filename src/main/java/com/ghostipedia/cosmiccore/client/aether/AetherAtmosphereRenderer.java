...
@OnlyIn(Dist.CLIENT)
public final class AetherAtmosphereRenderer {
    ...
    private AetherAtmosphereRenderer() {}
    ...
    public static boolean render(ClientLevel level, int ticks, float partialTick, Matrix4f modelViewMatrix,
        ...
        try {
            ...
            renderEarth(matrix);
            renderAetherSeaHorizon(level, partialTick, matrix, camera);
        } finally {
            RenderSystem.defaultBlendFunc();
    ...
    private static void renderDome(Matrix4f matrix, float solarAzimuth) {
        ShaderInstance shader = CosmicCoreClient.getAetherAtmosphereShader();
        if (shader != null) {
    ...
    private static void renderAetherSeaHorizon(ClientLevel level, float partialTick, Matrix4f matrix, Camera camera) {
        ShaderInstance shader = CosmicCoreClient.getAetherStormCurrentShader();
        if (shader == null) return;

        Vec3 cameraPosition = camera.getPosition();
        float passFade = AetherSightWallRenderer.seaVisibility(cameraPosition.y);
        if (passFade <= 0.01f) return;

        RenderSystem.setShader(() -> shader);
        var stormTimeUniform = shader.getUniform("StormTime");
        if (stormTimeUniform != null) {
            stormTimeUniform.set(AetherSightWallRenderer.stormTime(level.getGameTime(), partialTick));
        }
    ...
    private static SeaHorizonLayer[] horizonLayers(double cameraY) {
        return switch (AetherSightWallRenderer.layerOrder(cameraY)) {
            case 0 -> HORIZON_HIGH;
    ...
    private static void addSeaHorizonVertex(BufferBuilder buffer, Matrix4f matrix, Vec3 cameraPosition,
        ...
        float directionX = (float) Math.cos(angle);
        float directionZ = (float) Math.sin(angle);
        float layerBase = AetherSightWallRenderer.layerBase(layerDepth);
        float flatRelativeY = AetherSightWallRenderer.SEA_Y + layerBase - (float) cameraPosition.y;
    ...
    private static void renderAetherSea(Matrix4f matrix) {
        RenderSystem.setShader(GameRenderer::getPositionColorShader);
        BufferBuilder buffer = Tesselator.getInstance()
                .begin(VertexFormat.Mode.TRIANGLES, DefaultVertexFormat.POSITION_COLOR);
        int segments = 128;
    ...
    private static void renderAetherSeaWall(Matrix4f matrix) {
        RenderSystem.setShader(GameRenderer::getPositionColorShader);
        BufferBuilder buffer = Tesselator.getInstance()
